...
- 2022.03.21
    - 구글 로그인 화면 디자인 구현
        - 뭔가 대표 이미지 같은게 있었으면 좋겠다는 생각이 든다..
        - 앱을 쓰게할려면 디자인도 중요하다는 것을 느낌
        - 비슷한 todo list 앱 탐색
        - 앱 특성 상 위젯, 아이패드까지 추후에 고려해도 좋겠다는 생각이 들었음
- 2022.03.22
    - 구글 로그인 동작 버튼 터치 엑션이랑 연결
    - 구글 로그인 할 떄 토큰 저장하는 로직 추가할 필요가 있음
    - 구글 로그인 API를 호출하자마자 alert 띄우는 로직이 내부적으로 돌고 있어서 API 호출하는 시점에 main thread여야 함.
    - 지금 sdk 사용 방식 자체가 closure 안에 closure라서 이거 리펙터링 할 필요가 있음.
    - 로그인 성공 후의 동작이 필요함. rootViewController를 교체해줘야함.
- 2022.03.23
    - 구글 로그인 리펙터링 중
- 2022.03.24
    - 구글 로그인 리펙터링 및 기능 구현 중
- 2022.03.27
    - 구글 로그인 구현 완료
    - NavigationView 타이틀 제거 -> 커스텀해서 써야할 듯 함. 원인은 모르겠으나 Constraint error가 뜸. 
    - 데이터 연동하기 전에 디자인적인 요소를 먼저 해결해야할 듯 싶음.
        - 런치스크린, 아이콘, 컬러 컨셉 등등...
    - 아키텍처 관점에서도 생각을 해봐야할 듯 함.
- 2022.03.28
    - TCA 공부
- 2022.04.04
    - TCA 지속적으로 공부중
        - SwiftUI와 UIKit 모두 동일한 아키텍처로 갈 수 있어서 나쁘지 않다는 생각이 든다.
        - target이 13.0을 기준으로 해서 그런지 StateObject에 대한 활용은 전혀 없지만, 어차피 적절하게 섞어쓸 생각을 하면 크게 문제는 없을거라고 생각함.
        - 생각보다 이해하고 넘어가야할 개념들이 많아서 시간이 생각보다 더 걸릴 거라고 예상함.
- 2022.04.05
    - TCA 리펙터링 작업중
        - 대략적인 공부는 끝났다고 느끼고 있고, 디테일한 부분은 개발을 해가면서 챙길 예정
        - async/await와는 스타일이 정말 맞지 않는다는 생각이 들었음.
        - 적절하게 섞어쓰는게 좋다는 생각이 들었는데 특별한 상황 아니면 쓸 수 있는 케이스가 많지 않겠다는 생각이 든다.
        - 기존에 Environment로 side effect를 관리하는데, 이거에 대한 처리를 EffectInteractor가 담당한다고 정의함.
        - 단순하게 클로저를 주입하는 방식보단 좋은 방식이라고 생각함. (protocol로 주입)
...
- 2022.04.12
    - 테스트 케이스 작성중
        - 테스트할 때 사용하는 function에 대한 문서가 너무 미흡해서 일단 비슷하게 작성 후, xcode 내에서 구현부 코드 보고 분석중...
- 2022.05.23
    - 대략 5주동안, 기록이 없지만 회사일이 너무 바빴음.
        - 번아웃이와서 다시 회복하는데 시간도 조금 씀.
    - 그 동안 TCA를 다 까먹어서 코드 보면서 복습
    - 작업 해야될 것들 정리가 필요함.
